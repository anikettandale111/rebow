<?php
	...
	class OWTListTableClass extends WP_List_Table{
		...
		public function wp_list_table_data($orderby='',$order='',$search_term='',$order_type='',$order_status=''){
			...
			if(!empty($order_type)){
				echo $sql = "SELECT a.order_status as 'order_status',u.display_name AS 'display_name',b.order_id as 'order_id',date(b.created_at) AS 'order_date',u.user_email AS 'email',b.order_type as 'order_type',b.product_id as 'product_id', b.box_count as 'box_count',b.added_box_count as 'added_box_count',b.order_time_period as 'order_time_period',b.subtotal as 'subtotal',b.product_price as 'product_price',b.Delivery_Cost as 'Delivery_Cost',b.Pickup_Cost as 'Pickup_Cost',b.Sales_tax as 'Sales_tax',b.total_price as 'total_price',date(b.updated_at) as 'updated_date' from order_tracking a JOIN orders_data b ON a.order_id = b.order_id JOIN wp_users u ON a.user_id = u.ID WHERE b.active=1 AND a.active=1 and upper(b.order_type)='$order_type'";
			}
		...
		public function get_hidden_columns(){
			...
			$columns = array('delivery_address','pickup_address','product_id');
	...
	function owt_list_table_layout(){
		...
		echo "<form method='post' name='frm_search_post' action='" . $_SERVER['PHP_SELF'] . "?page=Table+List'>";
	...
	<script>
		jQuery('.ewc-filter-cat').change(function(){
		    ...
		    //alert(catFilter);
		        ...
		        document.location.href = 'admin.php?page=Table+List&order_type='+catFilter;    
		    //}
		});
		jQuery('.ewc-filter-cat1').change(function(){
		    ...
		    //alert(catFilter);
		        ...
		        document.location.href = 'admin.php?page=Table+List&order_status='+catFilter1;    
		    //}
		});
	</script>