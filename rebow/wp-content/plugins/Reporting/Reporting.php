...
function test_plugin_reporting_menu(){
    add_menu_page( 'Inventory Page', 'Inventory', 'manage_options', 'Inventory', 'test_init_inventory' );

}
add_action('admin_menu', 'test_plugin_sales_menu');

function test_plugin_sales_menu(){
    add_menu_page( 'Sales Page', 'Sales', 'manage_options', 'Sales', 'test_init_sales');
}
...
function test_init_inventory(){
	...
	echo "<table class='wp-list-table widefat fixed striped posts'>";
	echo "<tr><th>Boxes Out for Delivery</th><th>Boxes Out for Pickup</th><th>Boxes in Storage</th><th>Total Inventory</th></tr>";
	echo "<tr><td>$boxes_out_for_delivery</td><td>$boxes_out_for_pickup</td><td>$boxes_in_pack_mode</td><td>$total_inventory</td></tr></table>";
